...
class KGATv2(object):
    ...
    def _build_weights(self):
        ...
        all_weights['a_weight'] = tf.Variable(initializer([self.kge_dim, 1]))
        all_weights['W_weight'] = tf.Variable(initializer([self.kge_dim, self.kge_dim*2]))
        all_weights['b_weight'] = tf.Variable(initializer([self.kge_dim]))
    ...
    def _generate_transE_score(self, h, t, r):
        ...
        kg_score = tf.squeeze(tf.matmul(tf.nn.leaky_relu(tf.matmul(tf.concat([t_e, (h_e + r_e)], 
                                                                             axis=1), 
                                                                   tf.transpose(self.weights['W_weight'])) + self.weights['b_weight']), 
                                        self.weights['a_weight']))
