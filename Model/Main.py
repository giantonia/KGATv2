...
if __name__ == '__main__':
    ...
    *********************************************************
    ...
    if args.model_type in ['kgat', 'kgatv2', 'kgatv3', 'cfkg']:
    ...
    *********************************************************
    ...
    elif args.model_type in ['kgatv3']:
        model = KGATv3(data_config=config, pretrain_data=pretrain_data, args=args)
    ...
    *********************************************************
    ...
    if args.save_flag == 1:
        if args.model_type in ['bprmf', 'cke', 'fm', 'cfkg']:
            weights_save_path = '%sweights/%s/%s/l_r%s' % (args.weights_path, args.dataset, args.model_type, '-'.join([str(r) for r in eval(args.regs)]))

        elif args.model_type in ['ncf', 'nfm', 'kgat', 'kgatv2', 'kgatv3']:
            layer = '-'.join([str(l) for l in eval(args.layer_size)])
            weights_save_path = '%sweights/%s/%s/%s/l_r%s' % (
                args.weights_path, args.dataset, args.model_type, layer, '-'.join([str(r) for r in eval(args.regs)]))
    ...
    *********************************************************
    ...
    if args.pretrain == 1:
        if args.model_type in ['bprmf', 'cke', 'fm', 'cfkg']:
            pretrain_path = '%sweights/%s/%s/l_r%s' % (args.weights_path, args.dataset, args.model_type,
                                                             '-'.join([str(r) for r in eval(args.regs)]))

        elif args.model_type in ['ncf', 'nfm', 'kgat', 'kgatv2', 'kgatv3']:
            layer = '-'.join([str(l) for l in eval(args.layer_size)])
            pretrain_path = '%sweights/%s/%s/%s/l_r%s' % (
                args.weights_path, args.dataset, args.model_type, layer, '-'.join([str(r) for r in eval(args.regs)]))
    ...
    *********************************************************
    ...
    if args.report == 1:
        ...
        save_path = '%sreport/%s/%s.result' % (args.proj_path, args.dataset, args.model_type)
        ensureDir(save_path)
        f = open(save_path, 'w')
    ...
    *********************************************************
    ...
    for epoch in range(args.current_epoch, args.current_epoch + args.epoch):
        t1 = time()
        ...
        *********************************************************
        ...
        """
        if args.model_type in ['kgat', 'kgatv2', 'kgatv3']:
        ...
        if epoch % args.save_model == 0:
            ...
            print(f'save the weights of epoch {epoch} in path: ', weights_save_path)
        show_step = 10
        perf_str = 'Epoch %d [%.1fs]: train==[%.5f=%.5f + %.5f + %.5f]' % (
            epoch, time() - t1, loss, base_loss, kge_loss, reg_loss)
        with open(weights_save_path + '/logging.txt', 'a+') as f:
    ...
    f.write('epoch=%d, embed_size=%d, lr=%.4f, layer_size=%s, node_dropout=%s, mess_dropout=%s, regs=%s, adj_type=%s, use_att=%s, use_kge=%s, pretrain=%d\n\t%s\n'
            % (epoch, args.embed_size, args.lr, args.layer_size, args.node_dropout, args.mess_dropout, args.regs, args.adj_type, args.use_att, args.use_kge, args.pretrain, final_perf))
    f.close()
